...
class ContrailUtils(object):
    ...
    @staticmethod
    def get_analytics_nodes_from_cfgn(config_ip='127.0.0.1', config_port='8082', config_api=None):
        return_nodes = {}
        if not config_api:
            config_api = ContrailApi(ip=config_ip, port=config_port)
        global_configs_url = 'http://%s:%s/global-system-configs' % (config_api.get_ip(), config_api.get_port())
        ...
        for node in analytics_nodes:
            ...
            anode['port'] = 8081
            return_nodes['analytics_nodes'].append(anode)
        return return_nodes

    @staticmethod
    def get_config_api(config_ip='127.0.0.1', config_port='8082'):
        return ContrailApi(ip=config_ip, port=config_port)

    @staticmethod
    def get_analytics_ip_port(config_api):
        ip = port = None
        nodes = ContrailUtils.get_analytics_nodes_from_cfgn(config_api=config_api)
        analytics_nodes = nodes.get('analytics_nodes', [])
        if analytics_nodes:
            ip = analytics_nodes[0]['ip_address']
            port = analytics_nodes[0]['port']
        return ip, port
    
    @staticmethod
    def get_uve_obj(config_api):
        ip, port = ContrailUtils.get_analytics_ip_port(config_api)
        uve_obj = ContrailUVE(ip=ip, port=port)
        import pdb; pdb.set_trace()
        return uve_obj

    @staticmethod
    def get_vrouter_nodes_from_analytics(config_ip='127.0.0.1', config_port='8082', config_api=None):
        return_nodes = {}
        if not config_api:
            config_api = ContrailApi(ip=config_ip, port=config_port)
        uve_obj = ContrailUtils.get_uve_obj(config_api)
        ...
        for node in virtual_routers:
            ...
            return_nodes['vrouters'].append(vnode)
        return return_nodes


    @staticmethod
    def get_vrouter_nodes_from_cfgn(config_ip='127.0.0.1', config_port='8082', config_api=None):
        return_nodes = {}
        if not config_api:
            config_api = ContrailApi(ip=config_ip, port=config_port)
        global_configs_url = 'http://%s:%s/global-system-configs' % (config_api.get_ip(), config_api.get_port())
    ...
    @staticmethod
    def get_control_nodes_from_cfgn(config_ip='127.0.0.1', config_port='8082', config_api=None):
        return_nodes = {}
        if not config_api:
            config_api = ContrailApi(ip=config_ip, port=config_port)
        ...
        return_nodes = global_config['config_nodes']
        return_nodes['config_nodes'] = []
        for node in config_nodes:
            ...
            cnode['port'] = 8082
            return_nodes['config_nodes'].append(cnode)

        database_nodes = global_config['database_nodes']
        return_nodes['database_nodes'] = []
        for node in database_nodes:
            ...
            return_nodes['database_nodes'].append(dnode)
        ...
        for bgp_router in bgp_routers:
            ...
            if controller['bgp-router']['bgp_router_parameters']['vendor'] == 'contrail':
                ...
                return_nodes['control_nodes'].append(cnode)
        return return_nodes
    ...
    def get_contrail_info(uuid, uuid_type, config_ip='127.0.0.1', config_port='8082', **kwargs):
        ...
        if not analytics_ip:
            contrail_control_nodes = ContrailUtils.get_contrail_nodes(config_ip=config_ip, config_port=config_port)
    ...
    @staticmethod
    def get_contrail_vmi_info(fq_name, config_ip='127.0.0.1', config_port='8082', **kwargs):
        contrail_info = {'vrouter': [], 'control': []}
        analytics_ip = kwargs.get('analytics_ip', None)
        analytics_port = kwargs.get('analytics_port', '8081')
        ...
        try:
            if not analytics_ip:
                contrail_nodes = ContrailUtils.get_contrail_nodes(config_ip=config_ip, config_port=config_port, config_api=config_api)
    ...
    @staticmethod
    def get_contrail_vm_info(fq_name, config_ip='127.0.0.1', config_port='8082', **kwargs):
    ...
    @staticmethod
    def get_contrail_nodes(config_ip='127.0.0.1', config_port='8082', config_api=None):
        contrail_nodes = {}
        if not config_api:
            config_api = ContrailApi(ip=config_ip, port=config_port)
        global_configs_url = 'http://%s:%s/global-system-configs' % (config_ip, config_port)
